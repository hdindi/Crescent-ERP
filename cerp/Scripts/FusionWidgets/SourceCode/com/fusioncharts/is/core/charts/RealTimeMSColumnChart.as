/**
* @class RealTimeMSColumnChart
* @author InfoSoft Global(P) Ltd. www.InfoSoftGlobal.com
* @version 3.0
*
* Copyright(C) InfoSoft Global Pvt. Ltd. 2005-2006
* RealTimeMSColumnChart extends the SYRealTimeChart class to render the
* functionality of a 2D Multi-series Real-time column chart.
*/
//Import parent class
...
//Column 2D object
import com.fusioncharts.is.core.chartobjects.Column2D;
//External Interface - to expose methods via JavaScript
import flash.external.ExternalInterface;
class com.fusioncharts.is.core.charts.RealTimeMSColumnChart extends SYRealTimeChart {
	...
	* constructor.
	*/
	function RealTimeMSColumnChart(targetMC:MovieClip, depth:Number, width:Number, height:Number, x:Number, y:Number, debugMode:Boolean, registerWithJS:Boolean, DOMId:String, lang:String) {
		...
		this.log("Chart Type", "Real-time Multi Series 2D Column Chart", Logger.LEVEL.INFO);
		//List Chart Objects and set them in arrObjects array defined in super parent class.
		this.arrObjects = new Array("BACKGROUND", "CANVAS", "CAPTION", "SUBCAPTION", "XAXISNAME", "YAXISNAME", "DIVLINES", "YAXISVALUES", "HGRID", "DATALABELS", "DATAVALUES", "REALTIMEVALUE", "TRENDLINES", "TRENDVALUES", "DATAPLOT", "TOOLTIP", "VLINES", "VLINELABELS", "LEGEND");
	...
	*/
	public function render():Void {
		...
		if (this.numDS== 0)	{
			...
			//Update timer
			this.timeElapsed +=(this.params.animation) ? this.styleM.getMaxAnimationTime(this.objects.REALTIMEVALUE, this.objects.TRENDLINES, this.objects.TRENDVALUES, this.objects.VLINES, this.objects.DATAVALUES):0;
	...
	*	@param	ratio		Ratio of different colors in the fill.
			...
			if(arrDocElement [i].nodeName.toUpperCase() == "GRAPH" || arrDocElement [i].nodeName.toUpperCase() == "CHART") {
				...
				for(j = 0; j < arrLevel1Nodes.length; j ++){
					if(arrLevel1Nodes [j].nodeName.toUpperCase() == "CATEGORIES"){
						...
						this.dataset [this.numDS].color = getFV (dsAtts ["color"] , this.colorM.getColor());
						...
						//Dash Properties
						this.dataset [this.numDS].dashed = toBoolean (getFN (dsAtts["dashed"], Utils.fromBoolean(this.params.plotBorderDashed)));
	...
	*/
	private function parseAttributes(graphElement:XMLNode):Void{
		...
		this.params.plotBorderColor = formatColor (getFV (atts ["plotbordercolor"] , this.colorM.get2DPlotBorderColor()));
		...
		//Fill properties
		this.params.plotFillAngle = getFN (atts ["plotfillangle"] , 0);
		...
		//Show column shadows
		this.params.showColumnShadow = toBoolean (getFN (atts ["showshadow"] , atts ["showcolumnshadow"] , this.colorM.get2DShadow()));
	...
	*/
	private function setupAxis():Void {
		this.pAxis = new LinearAxis(this.params.yAxisMinValue, this.params.yAxisMaxValue, true, ! this.params.setAdaptiveYMin, this.params.numDivLines, this.params.yAxisValuesStep, this.nf, this.params.formatNumber, this.params.formatNumberScale, this.params.yAxisValueDecimals, this.params.forceYAxisDecimals);
	...
	*/
	private function allotDepths():Void{
		...
		//Line Chart
		this.dm.reserveDepths ("DATAPLOT", this.params.numDisplaySets * this.numDS);		
	...
	*/
	private function calculateXPoints()	{		
		//Loop variable
		var i : Number;
		//First up, calculate the spacing on canvas and individual column width
		var plotSpace : Number = (this.params.plotSpacePercent / 100) * this.elements.canvas.w;
		//Block Width
		var blockWidth : Number = (this.elements.canvas.w - plotSpace) / this.params.numDisplaySets;
		//Individual column space.
		var columnWidth : Number = blockWidth/this.numDS;
		/*
		//We do not check for max column width, as in real-time chart, users might
		//want to fill up the entire space available. However, if you wish to check
		//the max width, just uncomment the code below.
		...
		if (columnWidth > 50 && (this.params.plotSpacePercent == 20)){
			...
			plotSpace = this.elements.canvas.w - (this.params.numDisplaySets * blockWidth);
		}
		*/
		//Get space between two blocks
		var interBlockSpace : Number = plotSpace / (this.params.numDisplaySets + 1);
		//We finally have total plot space and column width - so store it
		...
		for (i = 1; i <= this.params.numDisplaySets; i++){
			this.dataPosX[i] = this.elements.canvas.x + (interBlockSpace * i) + (columnWidth * this.numDS * (i - 0.5));
		}		
	}
	/**
	...
	*/
	private function calculatePoints()	{
		...
		for (i=1; i<=this.numDS; i++){
			for (j=1; j<=this.num; j++){
				//Slot X - going back to start of this dataset's position - going forward by num columns.
				//this.dataset[i].data[j].x = this.dataPosX[(this.params.numDisplaySets-this.num)+j] - (this.config.columnWidth * 0.5 * this.numDS) + this.config.columnWidth*(i-0.5);
				//above reworded below - to reduced a step in calculation
				this.dataset[i].data[j].x = this.dataPosX[(this.params.numDisplaySets-this.num)+j] - (this.config.columnWidth*(this.numDS/2 - (i-0.5)));
				//Set y position
				this.dataset[i].data[j].y = this.config.basePlanePos;
				//Get end y position
				dataEndY = isNaN(this.dataset[i].data[j].value)?this.config.basePlanePos:this.pAxis.getAxisPosition(this.dataset[i].data[j].value, false);
				...
				this.dataset [i].data [j].w = (this.params.showColumnShadow && this.config.columnWidth > 2) ? (this.config.columnWidth - 1) : this.config.columnWidth;
	...
	*/
	private function drawColumns():Void{
		...
		for (i=1; i<=this.numDS; i++){
			...
			this.objM.removeGroupMC("DATAPLOT_" + i);
		}
		...
		for (i = 1; i <= this.numDS; i ++){
			for (j = 1; j <= this.num; j ++){
				...
				if (this.dataset [i].data [j].isDefined){
					...
					//Set the visibility as per lasts status of this dataset
					colMC._visible = dsVisible[i];								
					//Register with object manager
					this.objM.register(colMC,"PL_" + i + "_" + j,"DATAPLOT_"+i);
					...
					if (this.params.animation){
						this.styleM.applyAnimation (colMC, this.objects.DATAPLOT, this.macro, this.dataset [i].data [j].x, this.dataset [i].data [j].y, 100, 100, 100, null);
					}					
					//Apply filters
					colMC.filters = this.styleCache.dataPlotFilters;
		...
		if (dt["value"]!=undefined){
			...
			for (i=1; i<=numIncrements; i++) {
				...
				for (j=1; j<=this.numDS; j++){
					...
					this.dataset[j].data[this.num] = this.returnDataAsObject(setValue, setColor, this.dataset[j].alpha, this.dataset[j].ratio, setToolText, setLink, this.dataset[j].showValues, this.dataset[j].dashed);
