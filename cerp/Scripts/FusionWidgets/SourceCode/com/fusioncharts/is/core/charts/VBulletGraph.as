...
* VBulletGraph extends the BulletChart class to render the
* functionality of a vertical bullet graph.
*/
//Import parent class
...
class com.fusioncharts.is.core.charts.VBulletGraph extends BulletChart {
	/**
	* Constructor function. We invoke the super class'
	* constructor.
	*/
	function VBulletGraph(targetMC:MovieClip, depth:Number, width:Number, height:Number, x:Number, y:Number, debugMode:Boolean, registerWithJS:Boolean, DOMId:String, lang:String) {
		...
		this.log("Chart Type", "Vertical Bullet Graph", Logger.LEVEL.INFO);
	...
	*/
	private function parseAttributes(graphElement:XMLNode):Void {
		...
		//Padding
		this.params.captionPadding = getFN(atts["captionpadding"], 7);		
		...
		//Whether to show ticks at the right side of chart
		this.params.ticksOnRight = toBoolean(getFN(atts["ticksonright"], 0));
		...
		this.params.majorTMHeight = getFN(atts["majortmheight"], atts["majortmwidth"], 4);
	...
	*/
	private function setStyleDefaults():Void {
		...
		if (this.params.defaultAnimation){			
			...
			crYSAnim.name = "_SdColorRangeXScaleAnim";
			...
			//X-scale animation for color range
			var crXSAnim = new StyleObject ();
			crXSAnim.name = "_SdColorRangeYScaleAnim";
			...
			if (this.params.plotAsDot){
				//Simple y shift animation
				var plotYAnim = new StyleObject ();
				plotYAnim.name = "_SdPlotYAnim";
				plotYAnim.param = "_y";
				plotYAnim.easing = "regular";
				plotYAnim.wait = 0;
				plotYAnim.start = "$canvasEndY";
				plotYAnim.duration = 0.7;
				//Over-ride
				this.styleM.overrideStyle (this.objects.PLOT, plotYAnim, this.styleM.TYPE.ANIMATION, "_y");
				delete plotYAnim;
			}else{
				//YScale animation
				var plotYSAnim = new StyleObject ();
				plotYSAnim.name = "_SdPlotYScaleAnim";
				plotYSAnim.param = "_yscale";
				plotYSAnim.easing = "regular";
				plotYSAnim.wait = 0;
				plotYSAnim.start = 0;
				plotYSAnim.duration = 0.7;
				//Over-ride
				this.styleM.overrideStyle (this.objects.PLOT, plotYSAnim, this.styleM.TYPE.ANIMATION, "_yscale");
				delete plotYSAnim;
			}
			
			//Amimation for target - yshift
			var targetYAnim = new StyleObject ();
			targetYAnim.name = "_SdTargetYAnim";
			targetYAnim.param = "_y";
			targetYAnim.easing = "regular";
			targetYAnim.wait = 0;
			targetYAnim.start = "$canvasEndY";
			targetYAnim.duration = 0.7;
			//Over-ride
			this.styleM.overrideStyle (this.objects.TARGET, targetYAnim, this.styleM.TYPE.ANIMATION, "_y");
			delete targetYAnim;
		}
	}	
	/**
	...
	*/
	private function calculateCanvasPoints():Void{
		...
		//The (color range) can have the following objects group both left/right of it.		
		//Tick Padding + Tick Marks Height + Tick Value Padding + Tick Values Height
		//We'll calculate the left and right space required and then block the 
		...
		//Variables to store left and right space required
		...
		// ----------- CANVAS X & WIDTH PROPERTIES -----------//
		canvasStartX = this.params.chartLeftMargin+ tickLeftWidth;
		canvasWidth = this.width - (this.params.chartLeftMargin + this.params.chartRightMargin + tickLeftWidth + tickRightWidth);

		//Now, we calculate the vertical space that we've to block
		// --------------- DATA VALUE ---------------------//		
		var valueObj:Object;
		...
		if (this.params.showValue){
			...
			valueHeight = valueObj.height + this.params.valuePadding;
		}
		
		// -------------- CAPTION & SUB-CAPTION -------------------//
		var textFieldObj:Object;
		var captionHeight:Number = 0;
		var subCaptionHeight:Number = 0;
		var maxCaptionHeight:Number = 0;
		...
		if (this.params.caption!=""){
			...
			captionHeight = textFieldObj.height;
		}
		...
		if (this.params.subCaption!=""){
			...
			subCaptionHeight = textFieldObj.height;
		}
		maxCaptionHeight = captionHeight + subCaptionHeight;
		//Add caption padding, if either caption or sub-caption is to be shown
		if (maxCaptionHeight>0){
			maxCaptionHeight = maxCaptionHeight + this.params.captionPadding;
		}
		//Caption can take a maximum of 50% of available height
		maxCaptionHeight = Math.min(maxCaptionHeight, this.height/2);

		//---------------- CANVAS Y AND HEIGHT PROPERTIES -------------//
		canvasStartY = this.params.chartTopMargin + maxCaptionHeight;
		canvasHeight = this.height - (this.params.chartTopMargin + this.params.chartBottomMargin + maxCaptionHeight + valueHeight);
		...
		if (this.params.canvasTopMargin!=-1){
		...
		if (this.params.canvasBottomMargin!=-1){
			//Update height (deduct the difference)
	...
	*/
	private function calculatePoints():Void{
		...
		this.pAxis.setAxisCoords(this.elements.canvas.toY, this.elements.canvas.y);
		
		//Calculate the span x-range for each of the color range segment
		for (i=1; i<=this.numCR; i++) {
			this.colorR[i].fromY = this.pAxis.getAxisPosition((i==1)?this.pAxis.getMin():this.colorR[i].minValue);
			this.colorR[i].toY = this.pAxis.getAxisPosition((i==this.numCR)?this.pAxis.getMax():this.colorR[i].maxValue);
		}
	...
	*/
	private function drawColorRange(){
		...
		var colorRangeHeight:Number;
		var colorRangeRelativeY:Number;
		var borderColor:Number, borderAlpha:Number;
		//Set it's start position
		crMC._x = this.elements.canvas.x + this.elements.canvas.w/2;;
		crMC._y = this.elements.canvas.toY;
		//Create each color range with it's own properties, but inside the color range movie clip.
		for (i=1; i<=this.numCR; i++){
			//Height of the color range
			colorRangeHeight = this.colorR[i].toY - this.colorR[i].fromY;			
			colorRangeRelativeY = this.colorR[i].fromY - this.elements.canvas.toY;
			...
			var matrix:Object = {matrixType:"box", w:this.elements.canvas.w, h:colorRangeHeight, x:-(this.elements.canvas.w)/2, y:colorRangeRelativeY, r:0};
			...
			//Draw rounded rectangle
			DrawingExt.drawRoundedRect(crMC, -(this.elements.canvas.w)/2, colorRangeRelativeY, this.elements.canvas.w, colorRangeHeight, {tl:((i==this.numCR)?this.params.roundRadius:0), tr:((i==this.numCR)?this.params.roundRadius:0), bl:((i==1)?this.params.roundRadius:0), br:((i==1)?this.params.roundRadius:0)}, {l:borderColor, r:borderColor, t:borderColor, b:borderColor}, {l:borderAlpha, r:borderAlpha, t:borderAlpha, b:borderAlpha}, {l:this.params.colorRangeBorderThickness, r:this.params.colorRangeBorderThickness, b:this.params.colorRangeBorderThickness, t:this.params.colorRangeBorderThickness});			
			//End the fill.
			crMC.endFill();
			//-------------------------------------------------------------//			
		}
	...
	*/
	private function drawTicks():Void{
		...
		if (this.params.ticksOnRight){
			//Ticks on right - ticks outside - value outside
			ticksX = this.elements.canvas.toX + this.params.tickMarkDistance;
			multiplyF = 1;
			//Position for tick values
			tickValuesX = this.elements.canvas.toX + ((this.params.showTickMarks)?this.params.tickMarkDistance:0) + maxTickHeight + this.params.tickValueDistance;
			tickValueHAlign = "left";
		}else{
			//Ticks on left - ticks outside - value outside					
			ticksX = this.elements.canvas.x - this.params.tickMarkDistance;
			multiplyF = -1;			
			//Position for tick values
			tickValuesX = this.elements.canvas.x - ((this.params.showTickMarks)?this.params.tickMarkDistance:0) - maxTickHeight - this.params.tickValueDistance;
	...
	*/
	private function drawLabels():Void{
		//Container object
		var textFieldObj:Object;
		//Keeping a track of yShift for placing caption
		var yShift:Number = 0;
		...
		if (this.params.caption!=""){
			...
			textFieldObj = createText (false, this.params.caption, this.cMC, this.dm.getDepth("CAPTION"), this.width/2, this.params.chartTopMargin, 0, captionFont, false, 0, 0);			
			//Add to yShift
		...
		if (this.params.subCaption!=""){
			...
			textFieldObj = createText (false, this.params.subCaption, this.cMC, this.dm.getDepth("SUBCAPTION"), this.width/2, this.params.chartTopMargin + yShift, 0, subCaptionFont, false, 0, 0);
			//Apply animation and filters to tick marks
			if (this.params.animation){
	...
	*/
	private function drawPlot():Void{
		...
		//Plot width
		var plotWidth:Number = (this.params.plotFillPercent/100)*this.elements.canvas.w;
		...
		if (this.params.plotAsDot){
			...
			DrawingExt.drawRoundedRect(plotMC, -(plotWidth/2), -(plotWidth/2), plotWidth, plotWidth, {tl:0, tr:0, bl:0, br:0}, {l:plotBorderColor, r:plotBorderColor, t:plotBorderColor, b:plotBorderColor}, {l:this.params.plotBorderAlpha, r:this.params.plotBorderAlpha, t:this.params.plotBorderAlpha, b:this.params.plotBorderAlpha}, {l:this.params.plotBorderThickness, r:this.params.plotBorderThickness, b:this.params.plotBorderThickness, t:this.params.plotBorderThickness});
			//Shift the movie clip
			plotMC._x = this.elements.canvas.x + this.elements.canvas.w/2;
			plotMC._y = this.config.valuePos;
		}else{
			//We need to plot it as rectangle
			var rectHeight:Number = this.config.valuePos - this.config.basePos;
			//Draw the bar
			DrawingExt.drawRoundedRect(plotMC, -(plotWidth/2), 0, plotWidth, rectHeight, {tl:0, tr:0, bl:0, br:0}, {l:plotBorderColor, r:plotBorderColor, t:plotBorderColor, b:plotBorderColor}, {l:this.params.plotBorderAlpha, r:this.params.plotBorderAlpha, t:this.params.plotBorderAlpha, b:this.params.plotBorderAlpha}, {l:this.params.plotBorderThickness, r:this.params.plotBorderThickness, b:this.params.plotBorderThickness, t:this.params.plotBorderThickness});
			//Shift the movie clip
			plotMC._x = this.elements.canvas.x + this.elements.canvas.w/2;
			plotMC._y = this.config.basePos;
		}
		//------------- TOOL TIP ----------------//
	...
	*/
	private function drawTarget():Void{
		...
		if (this.showTarget){
			...
			//Target width
			var targetWidth:Number = (this.params.targetFillPercent/100)*this.elements.canvas.w;
			//Set line style
			targetMC.lineStyle(this.params.targetThickness, parseInt(this.params.targetColor,16), 100);
			targetMC.moveTo(-targetWidth/2, 0);
			targetMC.lineTo(targetWidth/2, 0);
			//Shift the entire line
			targetMC._x = this.elements.canvas.x + this.elements.canvas.w/2;
			targetMC._y = this.config.targetPos;
			//------------- TOOL TIP ----------------//
			if (this.params.showToolTip){
				//Function reference containers		
				var fnRollOver:Function;
	...
	*/
	private function drawValue():Void{
		...
		if (this.params.showValue){
			...
			valueObj = createText (false, this.valueDisplay, this.cMC, this.dm.getDepth("VALUE"), this.elements.canvas.x +  this.elements.canvas.w/2, this.elements.canvas.toY + this.params.valuePadding, 0, valueStyleObj, false, 0, 0);
			//Apply filter
