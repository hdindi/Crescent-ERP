...
* RealTimeMSLineChart extends the SYRealTimeChart class to render the
* functionality of a 2D Multi-series Real-time line chart.
*/
//Import parent class
...
class com.fusioncharts.is.core.charts.RealTimeMSLineChart extends SYRealTimeChart {
	...
	* constructor.
	*/
	function RealTimeMSLineChart(targetMC:MovieClip, depth:Number, width:Number, height:Number, x:Number, y:Number, debugMode:Boolean, registerWithJS:Boolean, DOMId:String, lang:String) {
		...
		this.log("Chart Type", "Real-time Multi Series 2D Line Chart", Logger.LEVEL.INFO);
	...
	*/
	private function setupAxis():Void {
		this.pAxis = new LinearAxis(this.params.yAxisMinValue, this.params.yAxisMaxValue, false, ! this.params.setAdaptiveYMin, this.params.numDivLines, this.params.yAxisValuesStep, this.nf, this.params.formatNumber, this.params.formatNumberScale, this.params.yAxisValueDecimals, this.params.forceYAxisDecimals);
	...
	private function calculatePoints()	{
		...
		for (i=1; i<=this.numDS; i++){
			for (j=1; j<=this.num; j++){
				...
				//Set y position
				this.dataset[i].data[j].y = isNaN(this.dataset[i].data[j].value)?0:this.pAxis.getAxisPosition(this.dataset[i].data[j].value, false);
		...
		if (dt["value"]!=undefined){
			...
			for (i=1; i<=numIncrements; i++) {
				...
				for (j=1; j<=this.numDS; j++){
					...
					this.dataset[j].data[this.num] = this.returnDataAsObject(setValue, setColor, this.dataset[j].alpha, setToolText, setLink, this.dataset[j].showValues, this.dataset[j].lineDashed, this.dataset[j].anchorSides, this.dataset[j].anchorRadius, this.dataset[j].anchorBorderColor, this.dataset[j].anchorBorderThickness, this.dataset[j].anchorBgColor, this.dataset[j].anchorAlpha, this.dataset[j].anchorBgAlpha);
