/**
* @class DYRealTimeChart
* @author InfoSoft Global (P) Ltd. www.InfoSoftGlobal.com
* @version 3.0
*
* Copyright (C) InfoSoft Global Pvt. Ltd. 2005-2006
* DYRealTimeChart extends RealTimeAxisChart class to encapsulate
* functionalities of a dual y axis chart with real time capabilities. 
...
class com.fusioncharts.is.core.DYRealTimeChart extends RealTimeAxisChart {
	//Reference to primary axis of the chart
	private var pAxis:LinearAxis;
	//Reference to secondary axis of the chart
	private var sAxis:LinearAxis;
	//Number formatting class for both the axis
	private var pnf:NumberFormatting;
	private var snf:NumberFormatting;
	//Containers for data.	
	//Array to store x-axis categories (labels)
	private var categories:Array;
	//Array to store datasets
	private var dataset:Array;
	//Array to store the divlines for both the axis
	...
	* constructor.
	*/
	function DYRealTimeChart(targetMC:MovieClip, depth:Number, width:Number, height:Number, x:Number, y:Number, debugMode:Boolean, registerWithJS:Boolean, DOMId:String, lang:String) {
	 ...
	 * for the class.
	 *	@param		isPrimary			Whether we are setting for the primary axis
	 ...
	 *	@return							Nothing
	*/
	private function setupNumberFormatting(isPrimary:Boolean, numberPrefix:String, numberSuffix:String, scaleRecursively:Boolean, maxScaleRecursion:Number, scaleSeparator:String, defaultNumberScale:String, numberScaleValues:String, numberScaleUnits:String, decimalSeparator:String, thousandSeparator:String, inDecimalSeparator:String, inThousandSeparator:String):Void {
		//Setup the number formatting instance for the required axis
		if (isPrimary){
			this.pnf = new NumberFormatting(numberPrefix, numberSuffix, scaleRecursively, maxScaleRecursion, scaleSeparator, defaultNumberScale, numberScaleValues, numberScaleUnits, decimalSeparator, thousandSeparator, inDecimalSeparator, inThousandSeparator);
		}else{
			this.snf = new NumberFormatting(numberPrefix, numberSuffix, scaleRecursively, maxScaleRecursion, scaleSeparator, defaultNumberScale, numberScaleValues, numberScaleUnits, decimalSeparator, thousandSeparator, inDecimalSeparator, inThousandSeparator);
		}
	}	
	/**
	* returnDataAsSDivLine method returns the data provided to the method
	* as a secondary axis div line object.
	...
	*/
	private function returnDataAsSDivLine(value:Number, showValue:Boolean):Object {
		//Create a new object
		var divLineObject = new Object();
		//Set numerical value
		divLineObject.value = value;
		//Set display value - formatted number.
		divLineObject.displayValue = this.snf.formatNumber(value, this.params.sFormatNumber, this.params.sFormatNumberScale, this.params.sDecimals, this.params.sForceDecimals);
	...
	*	@param	parentYAxis		Which axis does the trendline belong to?
	...
	*/
	private function returnDataAsTrendObj(parentYAxis:String, startValue:Number, endValue:Number, displayValue:String, color:String, thickness:Number, alpha:Number, isTrendZone:Boolean, showOnTop:Boolean, isDashed:Boolean, dashLen:Number, dashGap:Number, valueOnRight:Boolean):Object{
	...
	private function parseTrendLineXML (arrTrendLineNodes:Array):Void {
		...
		for (i = 0; i <= arrTrendLineNodes.length; i ++){
			...
			if (arrTrendLineNodes [i].nodeName.toUpperCase () == "LINE"){
				...
				try{
					startValue = this.pnf.parseValue(getFV(lineAttr["startvalue"], lineAttr["value"]));
				}catch (e:Error){
				...
				try{
					endValue = this.pnf.parseValue(getFV(lineAttr["endvalue"], startValue));					
				}catch (e:Error){
					//If the value is not a number, log a data
		...
		for (i=1; i <=this.numTrendLines; i++){
			...
			if (isNaN(this.trendLines[i].startValue) || isNaN(this.trendLines[i].endValue)){
				...
				if (this.trendLines[i].parentYAxis=="P"){
					this.trendLines [i].displayValue = getFV(this.trendLines[i].displayValue, this.pnf.formatNumber(this.trendLines[i].startValue, this.params.formatNumber, this.params.formatNumberScale, this.params.yAxisValueDecimals, this.params.forceYAxisDecimals));
				}else{
					this.trendLines [i].displayValue = getFV(this.trendLines[i].displayValue, this.snf.formatNumber(this.trendLines[i].startValue, this.params.sFormatNumber, this.params.sFormatNumberScale, this.params.sYAxisValueDecimals, this.params.forceSYAxisDecimals));
				}
			}
		}
	...
	private function validateTrendLines():Void{
		...
		for (i = 1; i <= this.numTrendLines; i ++){
			...
			if (this.trendLines [i].parentYAxis=="P"){
				...
				if ((this.trendLines [i].startValue<this.sAxis.getYMin()) || (this.trendLines [i].startValue > this.sAxis.getYMax()) || (this.trendLines [i].endValue < this.sAxis.getYMin()) || (this.trendLines[i].endValue>this.sAxis.getYMax())){
					//Invalidate it
					this.trendLines [i].isValid = false;
				} else {
					//Mark it as valid
	...
	private function calculateCanvasCoords():Void{
		...
		for (i = 1; i < this.pDivLines.length; i ++){
			...
			if (this.pDivLines [i].showValue){
				...
				if ((i == 1) || (i == this.pDivLines.length - 1)){
		...
		var trendObj : Object;
		for (i = 1; i <= this.numTrendLines; i ++){
			if (this.trendLines [i].isValid == true && this.trendLines [i].parentYAxis == "P"){
				//If it's a valid trend line and value is to be shown on left (primary axis)
				//Get the width of the text
		...
		for (i = 1; i <= this.numTrendLines; i ++){
			if (this.trendLines [i].isValid == true && this.trendLines [i].parentYAxis == "S"){
				//If it's a valid trend line and value is to be shown on right (secondary axis)
				//Get the width of the text
				trendObj = createText (true, this.trendLines [i].displayValue, this.tfTestMC, 1, testTFX, testTFY, 0, trendStyle, false, 0, 0);
				//Accomodate
		...
		delete trendObj;		
		//Accomodate max of secondary axis div lines / trend values
		this.config.rightSideValueWidth = 0;
		...
		this.sAxis.setAxisCoords(this.elements.canvas.y, this.elements.canvas.toY);
	}
	/**
	...
	private function calcTrendLinePos():Void{
		...
		for (i = 1; i <= this.numTrendLines; i ++){
			...
			if (this.trendLines [i].isValid == true){
				...
				this.trendLines [i].y = (this.trendLines [i].parentYAxis=="P")?(this.pAxis.getAxisPosition (this.trendLines[i].startValue, false)):(this.sAxis.getAxisPosition (this.trendLines[i].startValue, false));
				...
				if (this.trendLines [i].startValue != this.trendLines [i].endValue)	{
					...
					this.trendLines [i].toY = (this.trendLines [i].parentYAxis=="P")?(this.pAxis.getAxisPosition (this.trendLines[i].endValue, false)):(this.sAxis.getAxisPosition (this.trendLines[i].endValue, false));
	...
	*/
	private function drawHeaders (){
		...
		if (this.params.PYAxisName != "")
		{
			...
			if (this.params.rotateYAxisName)
			{
				var yAxisNameObj : Object = createText (false, this.params.PYAxisName, this.cMC, this.dm.getDepth ("PYAXISNAME") , this.params.chartLeftMargin, this.elements.canvas.y + (this.elements.canvas.h / 2) , 270, yAxisNameStyleObj, false, 0, 0);
				//Apply animation
				if (this.params.animation)
				{
			...
			}else
			{
				...
				var yAxisNameObj : Object = createText (false, this.params.PYAxisName, this.cMC, this.dm.getDepth ("PYAXISNAME") , this.params.chartLeftMargin, this.elements.canvas.y + (this.elements.canvas.h / 2) , 0, yAxisNameStyleObj, true, this.params.PYAxisNameWidth + 1, this.elements.canvas.h);
				//Apply animation
				if (this.params.animation)
				{
		...
		if (this.params.SYAxisName != "")
		{
			...
			//If the name is to be rotated
			if (this.params.rotateYAxisName){
				var yAxisNameObj : Object = createText (false, this.params.SYAxisName, this.cMC, this.dm.getDepth ("SYAXISNAME"), this.elements.canvas.toX + this.params.yAxisValuesPadding + this.config.rightSideValueWidth + this.params.yAxisNamePadding, this.elements.canvas.y + (this.elements.canvas.h / 2) , 270, yAxisNameStyleObj, false, 0, 0);
				yAxisNameObj.tf._rotation = 90;
				yAxisNameObj.tf._y -= yAxisNameObj.tf._height;
				yAxisNameObj.tf._x += yAxisNameObj.tf._width;
				//Apply animation
				if (this.params.animation){
					this.styleM.applyAnimation (yAxisNameObj.tf, this.objects.YAXISNAME, this.macro, yAxisNameObj.tf._x, yAxisNameObj.tf._y , 100, null, null, null);
				}
			}else{
				//We show horizontal name
				//Adding 1 to this.params.PYAxisNameWidth and then passing to avoid line breaks
				var yAxisNameObj : Object = createText (false, this.params.SYAxisName, this.cMC, this.dm.getDepth ("SYAXISNAME"), this.elements.canvas.toX + this.params.yAxisValuesPadding + this.config.rightSideValueWidth + this.params.yAxisNamePadding, this.elements.canvas.y + (this.elements.canvas.h / 2) , 0, yAxisNameStyleObj, true, this.params.SYAxisNameWidth + 1, this.elements.canvas.h);
				//Apply animation
				if (this.params.animation)
				{
					this.styleM.applyAnimation (yAxisNameObj.tf, this.objects.YAXISNAME, this.macro, yAxisNameObj.tf._x, yAxisNameObj.tf._y, 100, null, null, null);
	...
	private function drawDivLines():Void{
		...
		for (i = 0; i < this.pDivLines.length; i ++){			
			...
			if ((i == 0) || (i == this.pDivLines.length - 1)){
				if (this.params.showLimits && this.pDivLines [i].showValue){
					depth++;
					//Get y position for textbox
					yPos = this.pAxis.getAxisPosition (this.pDivLines[i].value, false);
					//Create the limits text
					divLineValueObj = createText (false, this.pDivLines [i].displayValue, this.cMC, depth, this.elements.canvas.x - this.params.yAxisValuesPadding, yPos, 0, divLineFontObj, false, 0, 0);
					//Register it with object manager
					this.objM.register(divLineValueObj.tf, "DIVVALUE_"+i, "DIVVALUES");					
				}
			} else if (this.pDivLines [i].value == 0){
				...
				if (this.params.showZeroPlane){
					...
					//So, check if we've to show div line values
					if (this.params.showDivLineValues && this.pDivLines[i].showValue){
						//Create the text
						divLineValueObj = createText (false, this.pDivLines[i].displayValue, this.cMC, zpVDepth, this.elements.canvas.x - this.params.yAxisValuesPadding, yPos, 0, divLineFontObj, false, 0, 0);						
		...
		//Also, create the secondary axis values
			...
			if (this.sDivLines[i].showValue){				
				...
				if ((i == 0) || (i == this.pDivLines.length - 1)){
					if (this.params.showSecondaryLimits){
						depth++;
						//Get y position for textbox
						yPos = this.sAxis.getAxisPosition (this.sDivLines[i].value, false);
						//Create the limits text
						...
						//Register it with object manager
						this.objM.register(divLineValueObj.tf, "DIVVALUE_S"+i, "DIVVALUES");					
					}
				}  else {
					//It's a div interval - div line
					//Get y position
				...
				if (this.sDivLines[i].showValue){
					if (this.params.animation){
						this.styleM.applyAnimation (divLineValueObj.tf, this.objects.YAXISVALUES, this.macro, divLineValueObj.tf._x, divLineValueObj.tf._y, 100, null, null, null);
					}
					//Apply filters
					divLineValueObj.tf.filters = this.styleCache.divLineValueFilters;
				}
	...
	private function drawHGrid():Void 	{
		...
		if (this.params.showAlternateHGridColor && this.pDivLines.length > 3){
			...
			for (i=1; i<this.pDivLines.length-1; i=i+2){
				//Get y position
				yPos = this.pAxis.getAxisPosition(this.pDivLines[i].value, false);
				yPosEnd = this.pAxis.getAxisPosition(this.pDivLines[i + 1].value, false);
				//Set fill color
