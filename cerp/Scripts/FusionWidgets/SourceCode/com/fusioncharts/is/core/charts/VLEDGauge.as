...
* VLEDGauge extends the RealTimeGauge class to render the
* functionality of a vertical LED gauge.
*/
//Import parent class
...
class com.fusioncharts.is.core.charts.VLEDGauge extends RealTimeGauge {
	//Value of the chart.
	...
	* constructor.
	*/
	function VLEDGauge(targetMC:MovieClip, depth:Number, width:Number, height:Number, x:Number, y:Number, debugMode:Boolean, registerWithJS:Boolean, DOMId:String, lang:String) {
		...
		this.log("Chart Type", "Real-time Vertical LED Gauge", Logger.LEVEL.INFO);
	...
	*/
	private function calculateGaugePoints():Void{
		...
		//In vertical LED gauge, we can have the tick marks/values on the left or right of gauge
		...
		//----------------------------------------------------------------------------//
		//We finally have the maximum height allocation to be alloted
		//Now, create the gauge element accordingly.
		this.elements.gauge = this.returnDataAsElement(this.params.chartLeftMargin+tickLeftWidth, this.params.chartTopMargin, this.width-(this.params.chartLeftMargin + this.params.chartRightMargin + tickLeftWidth + tickRightWidth), this.height-(this.params.chartTopMargin + this.params.chartBottomMargin + valueHeight));		
	}
	/**
	...
	*/
	private function calculatePoints():Void{
		...
		this.config.numBars = int(this.elements.gauge.h/(this.params.ledSize + this.params.ledGap));
	}
	/**
	...
	*/
	private function drawGauge(){
		...
		gaugeMC._x = this.elements.gauge.x + (this.elements.gauge.w/2);
		gaugeMC._y = this.elements.gauge.toY;
	...
	*/
	private function drawLED(firstTime:Boolean):Void{
		...
		barWidth = this.elements.gauge.w;
		barSpacing = this.params.ledGap;		
		barHeight = this.params.ledSize;
		//If the height available according to specified barHeight is less, we increase the height
		//so as to fill the entire canvas.
		if ((this.config.numBars*barHeight)+(barSpacing*(this.config.numBars-1))<(this.elements.gauge.h-this.params.gaugeBorderThickness)) {
			//Re-set the bar height, so that the entire gauge is filled
			barHeight = ((this.elements.gauge.h-this.params.gaugeBorderThickness)-(barSpacing*(this.config.numBars-1)))/(this.config.numBars);
		}
		//Define the starting point
		var currYIndex:Number, colorRange:Number, colorCode:Number, colorAlpha:Number;
		currYIndex = this.elements.gauge.toY - (this.params.gaugeBorderThickness/2);
		...
		for (i=1; i<=this.config.numBars; i++) {
			...
			for (j=1; j<=this.numCR; j++) {
				if (((currYIndex+(barHeight/2))<=this.colorR[j].fromY) && ((currYIndex+(barHeight/2))>=this.colorR[j].toY) && i<=displayBars) {
					colorCode = parseInt(this.colorR[((this.params.useSameFillColor)?(this.getValueCRIndex()):(j))].color, 16);
					colorAlpha = 100;
				} else if (((currYIndex+(barHeight/2))<=this.colorR[j].fromY) && ((currYIndex+(barHeight/2))>=this.colorR[j].toY) && i>displayBars) {
			...
			DrawingExt.drawRoundedRect(mcLED, this.elements.gauge.x + (this.params.gaugeBorderThickness/2), currYIndex, this.elements.gauge.w-this.params.gaugeBorderThickness, barHeight, {tl:0, tr:0, bl:0, br:0}, {l:0, r:0, t:0, b:0}, {l:0, r:0, t:0, b:0}, {l:0, r:0, b:0, t:0});
			mcLED.endFill();
			//Decrement y pointer
			currYIndex -= (barHeight+barSpacing);
		}
	...
	*/
	private function drawValue(firstTime:Boolean):Void{
		...
		if (this.params.showValue){
			...
			valueObj = createText (false, this.nf.formatNumber(this.value, this.params.formatNumber, this.params.formatNumberScale, this.params.decimals, this.params.forceDecimals), this.cMC, depth, this.elements.gauge.x + (this.elements.gauge.w/2), this.elements.gauge.toY + this.params.valuePadding, 0, valueStyleObj, false, 0, 0);
			//Apply filter
