...
class com.fusioncharts.is.core.StyleManager{
	//Create enumeration for the various style type that's supported
	public var TYPE:FCEnum;
	//Enumeration to store the list of chart objects
	...
	/**
	 * Constructor function that initiates the StyleManager class.
	 *	@param	chartIns	Instance of chart class for which we're initializing
	 *						this style manager. We need this reference to log any error
	 *						messages. We cannot throw, as there can be multiple errors logged.	 
	 *	@param	chartObjs	Enumeration of chart objects.
	*/
	function StyleManager(chartIns:Chart, chartObjs:FCEnum){
		//Store chart instance
		this.chartIns = chartIns;
		//Store list of chart objects
	...
	* elements (application and definition). 
	*	@param	arrStyleNodes	Array of XML Nodes containing style definition. 
	*							Array contains child elements of STYLE element.
	*	@return					Nothing
	*/
	public function parseXML(arrStyleNodes:Array):Void{
		//Loop variables
		var k : Number;
		var l : Number;
		var m : Number;
			...
			if (arrStyleNodes [k].nodeName.toUpperCase () == "DEFINITION"){
				...
				for (l = 0; l < arrDefNodes.length; l ++){
					...
					if (arrDefNodes [l].nodeName.toUpperCase () == "STYLE"){
						...
						var styleAttr:Array = Utils.getAttributesArray (styleNode)
						...
						try {
							...
							var styleTypeId:Number = this.getStyleTypeId(styleTypeName);
							...
							//Remember: StyleObject is a dynamic class, and we can add properties
							//at run-time.
							...
							this.addStyle(styleName, styleTypeId, styleObj);
			...
			if (arrStyleNodes [k].nodeName.toUpperCase () == "APPLICATION"){
				...
				for (l = 0; l < arrAppNodes.length; l ++){
					...
					if (arrAppNodes [l].nodeName.toUpperCase () == "APPLY"){
						...
						var appAttr:Array = Utils.getAttributesArray(arrAppNodes [l]);
						...
						if (isChartObject(toObject)){
							...
							var objectId = this.chartObjs.getItem(toObject);
							...
							for (m = 0; m < arrStyles.length; m ++){
								try{
									...
									this.associateStyle (objectId, arrStyles[m]);
								}catch (e : com.fusioncharts.is.helper.FCError){
									//If the control comes here, that means the given style name
									//is undefined. So, we log the error message to the logger.
									this.chartIns.log (e.name, e.message, e.level);
								}
							}
						} else{
							this.chartIns.log ("Invalid Chart Object in Style Definition", "\"" + toObject + "\" is not a valid Chart Object. Please see the list above for valid Chart Objects.", Logger.LEVEL.ERROR);
						}
					}
				}
			}
	...
	*/
	...
	// ------------ Functions to help over-ride styles --------------//	
	/**
	...
	* @param	defaultObject	Default style object that we want to use when
	*							the user has not specified a property.
	* @param	intStyleType	What type of style are we over-riding?
	* @param	animationParam	If we're over-riding an animation style type, this
	*							attribute defines which property of animation are we
	*							over-riding.
	...
	public function overrideStyle(chartObject:Number, defaultObject:StyleObject, intStyleType:Number, animationParam:String):StyleObject{
		...
		if (objectStyle.isDefined == true){
			...
			*/
			...
			var styleName : String = Utils.getFirstValue(objectStyle.name, defaultObject.name)
			styleName = styleName.toUpperCase();
			styleName = (styleName == "") ? (String ("_S" + getTimer ())) : (styleName);
			...
			//Create a copy of the defaultObject, so that if the same style object is
			//applied to multiple chart objects, they will retain individual properties,
			var newStyle:StyleObject = new StyleObject();
			var item:Object;
			for (item in defaultObject){
				newStyle[item]= defaultObject[item];
			}			
			//First, check if a name has been defined for this style. If not
			//attach an arbitrary name. If it has been defined also, we add styles length
			//at the end, so as to differentiate between two styles.
			var styleName:String = newStyle.name;
			styleName = (styleName == undefined) ? (String("_S"+getTimer()+"_") + String(this.styles.size()+1)):(styleName + String(this.styles.size()+1));
	...
	*/
	...
	// ----------- Style Accessor and Application methods -----------//
	/**
	...
	*/
	public function getStyle (chartObject:Number, intStyleType:Number, animationParam:String):StyleObject {
		...
		if (intStyleType == this.TYPE.ANIMATION){
			//List all the animation styles for this object and compare
		...
		for (i = 0; i < arguments.length; i ++){
			...
			for (j=0;j<objStyles.length; j ++){
				...
				if (this.styleType[objStyles[j]] == this.TYPE.ANIMATION){
					...
					//Also, account for wait factor, if defined.
	...
	*	@param	macroIns 	Reference to macro object for evaluation of macros.
	*	@param	dX			End value of x animation. Null to disable.
	*	@param	dY			End value of y animation. Null to disable.
	*	@param	dAlpha		End value of alpha transition. Null to disable.
	*	@param	dXScale		End value of x-scale animation. Null to disable.
	*	@param	dXScale		End value of y-scale animation. Null to disable.
	*	@param	dRotation	End value of rotational motion. Null to disable.
	*/
	public function applyAnimation (mc:MovieClip, chartObject:Number, macroIns:Macros, dX:Number, dY:Number, dAlpha:Number, dXScale:Number, dYScale:Number, dRotation:Number):Void {
			...
			for (i=0; i<animationStyles.length; i ++){
				...
				if (!(end == null || end == undefined || isNaN (end)) && !(start == end)){
					...
					if (animationStyles[i].wait != undefined && animationStyles [i].wait!= null && (Number(animationStyles[i].wait) > 0)){
						...
						//accessible inside onMotionFinished event handler function - and for this  
						//tween instance only. If we do not store the parameters locally, the tween 
						//class "remembers" parameters from previous calls (because of var scoping).
						tweens[i].obj.start = start;
						tweens[i].obj.end = end;
						tweens[i].obj.duration = duration;
						//Now, after the initial motion is finished, we do the wait part animation.
						tweens[i].onMotionFinished = function(){
							//Since the waiting period is now over, we can
							//change the visual property
							this.continueTo(this.obj.end, this.obj.duration);							
	...
	/**
	 * getFilterStyles method returns the filter styles for a particular object to be applied
	 * on the given movie clip.
	 *	@param	mc			Movie clip to which we've to apply filter
	*	@param	chartObject	The numeric id of chart object which is being
	*						represented by this movie clip
	*	@param	arrNotApply	Array containing ids (types) of filter types not to apply.
	*						Specifically useful when you do not want to apply a particular
	*						filter type.
	*	@return				Array containing the styles applicable to the movie clip.
	*/
	public function getFilterStyles(chartObject:Number, arrNotApply:Array):Array{
		//First up, we need to get a list of filters defined for this chart object.
		//Get the list of styles applicable for this object
		var objStyles:Array = new Array();
		objStyles = this.objectStyle[chartObject];
		var i:Number;
		var filterStyles:Array = new Array();
		//Also, create an array to store the filter objects for mc
		var mcFilters:Array = new Array ();
		for (i = 0; i < objStyles.length; i ++){
		...
		if (filterStyles.length>0){
			...
			for (i = 0; i < filterStyles.length; i ++){
				...
				switch (filterStyles [i].type){
					...
					*			360˚ (floating point). The default value is 45.
		...
		delete filters;
		//Return the array containing styles.
		return mcFilters;
	}
	/**
	...
	*/
	public function applyFilters(mc:MovieClip, chartObject:Number, arrNotApply:Array):Void {		
		//Retrieve and apply the filters to the movie clip
		mc.filters = this.getFilterStyles(chartObject, arrNotApply);		
	}
	// ------------------- Validator functions ----------------------//
	/**
	...
	*/
	private function isChartObject(strObjName:String):Boolean {
		return (this.chartObjs.getItem(strObjName)!=-1);
	}
	/**
	...
	*/
	public function getStyleTypeId(strStyleType:String):Number {
		...
		typeId = this.TYPE.getItem(strStyleType);
		//If the type is not defined, it's returned as -1. So check it and log the error
		if (typeId == -1){
			//Throw an error
