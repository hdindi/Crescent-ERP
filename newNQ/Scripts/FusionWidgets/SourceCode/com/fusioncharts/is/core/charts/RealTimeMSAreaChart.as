/**
* @class RealTimeMSAreaChart
* @author InfoSoft Global(P) Ltd. www.InfoSoftGlobal.com
* @version 3.0
*
* Copyright(C) InfoSoft Global Pvt. Ltd. 2005-2006
* RealTimeMSAreaChart extends the SYRealTimeChart class to render the
* functionality of a 2D Multi-series Real-time area chart.
*/
//Import parent class
...
class com.fusioncharts.is.core.charts.RealTimeMSAreaChart extends SYRealTimeChart {
	...
	* constructor.
	*/
	function RealTimeMSAreaChart(targetMC:MovieClip, depth:Number, width:Number, height:Number, x:Number, y:Number, debugMode:Boolean, registerWithJS:Boolean, DOMId:String, lang:String) {
		...
		this.log("Chart Type", "Real-time Multi Series 2D Area Chart", Logger.LEVEL.INFO);
	...
	private function calculatePoints()	{
		...
		for (i=1; i<=this.numDS; i++){
			for (j=1; j<=this.num; j++){
				...
				//Set y position
				this.dataset[i].data[j].y = isNaN(this.dataset[i].data[j].value)?this.config.basePlanePos:this.pAxis.getAxisPosition(this.dataset[i].data[j].value, false);
