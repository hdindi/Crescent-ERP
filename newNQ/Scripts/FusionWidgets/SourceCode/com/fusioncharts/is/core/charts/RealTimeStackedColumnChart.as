...
* RealTimeStackedColumnChart extends the SYRealTimeChart class to render the
* functionality of a 2D Real-time Stacked column chart.
*/
//Import parent class
...
class com.fusioncharts.is.core.charts.RealTimeStackedColumnChart extends SYRealTimeChart {
	//Array to store Sums
	...
	* constructor.
	*/
	function RealTimeStackedColumnChart(targetMC:MovieClip, depth:Number, width:Number, height:Number, x:Number, y:Number, debugMode:Boolean, registerWithJS:Boolean, DOMId:String, lang:String) {
	...
	*/
	private function parseAttributes(graphElement:XMLNode):Void{
		...
		// --------------------- CONFIGURATION ------------------------- //
		//Upper limits of y axis
		//No lower limit in stacked chart.
		...
		//We do not set interactive legend for stacked legends
	...
	*/
	private function allotDepths():Void{
		...
		//Line Chart
		this.dm.reserveDepths ("DATAPLOT", this.params.numDisplaySets);		
		...
		//Data Sums
		this.dm.reserveDepths ("DATASUM", this.params.numDisplaySets);
	...
	*/
	private function calculateXPoints()	{		
		...
		var columnWidth : Number = (this.elements.canvas.w - plotSpace) / this.params.numDisplaySets;
		/*
		...
		*/
		...
		var interColSpace : Number = plotSpace / (this.params.numDisplaySets + 1);
		...
		for (i = 1; i <= this.params.numDisplaySets; i++){
			this.dataPosX[i] = this.elements.canvas.x + (interColSpace * i) + (columnWidth * (i - 0.5));
		}		
	}
	/**
	...
	private function calculatePoints()	{
		...
		for (i=1; i<=this.num; i++){
			...
			for (j=1; j<=this.numDS; j++){
				this.dataset[j].data[i].x = this.dataPosX[(this.params.numDisplaySets-this.num)+i];				
				//Set width
				this.dataset[j].data[i].w = this.config.columnWidth;
				//Height for each column
				dataEndY = this.pAxis.getAxisPosition(this.dataset[j].data[i].value);
	...
	*/
	private function drawColumns():Void{
		...
		for (i = 1; i <= this.num; i ++){
			...
			this.objM.register(stackMC,"PL_" + i,"DATAPLOT");
			//Set position
			stackMC._x = this.dataPosX[this.params.numDisplaySets-this.num+i];
			...
			for (j = 1; j <= this.numDS; j ++){
				...
				if (this.dataset [j].data [i].isDefined){
					...
					if (this.params.showToolTip){
						...
						fnRollOver.text = this.dataset[j].data[i].toolText;
					...
					if (this.dataset [j].data[i].link != "" && this.dataset[j].data[i].link != undefined && this.params.clickURL == ""){
						...
						//Set link itself
						fnClick.link = this.dataset[j].data[i].link;
						//Assign
			...
			if (this.params.animation){
				this.styleM.applyAnimation (stackMC, this.objects.DATAPLOT, this.macro, stackMC._x, stackMC._y, 100, 100, 100, null);
			}
			//Apply filters
			stackMC.filters = this.styleCache.dataPlotFilters;
			//Increase depth
			depth ++;
		}		
	...
	*/
	private function drawValues():Void {
		...
		this.objM.removeGroupTF("DATAVALUES");
		...
		for (i = 1; i <= this.numDS; i ++){
			for (j = 1; j <= this.num; j ++){
				...
				if (this.dataset [i].data [j].isDefined && this.dataset [i].data [j].showValue) {
					...
					//Register
					this.objM.register(valueObj.tf,"VL"+i+"_"+j, "DATAVALUES");					
					//Apply filter
	...
	*/
	private function drawSum():Void{
		if (this.params.showSum){
			//Clear the text fields
			this.objM.removeGroupTF("DATASUM");		
			//Get value text style
			...
			for (i = 1; i <= this.num; i ++){
				...
				if (this.sums [i].sum != 0){
					...
					valueObj = createText (false, this.nf.formatNumber(this.sums [i].sum, this.params.formatNumber, this.params.formatNumberScale, this.params.decimals, this.params.forceDecimals), this.cMC, depth,  this.dataPosX[this.params.numDisplaySets-this.num+i], yPos, angle, valueStyleObj, false, 0, 0);
					//Register
					this.objM.register(valueObj.tf,"SUM"+i, "DATASUM");
					//Apply filter
					valueObj.tf.filters = this.styleCache.dataValueFilters;
					//Apply animation
					if (this.params.animation){
